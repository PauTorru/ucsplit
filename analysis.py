...
def bob_angle(pos):
    ...
    _B1 = B1 - O1
    _B2 = B2 - O1
    angle1 = (180.0 / np.pi) * np.arccos(
        np.dot(_B1, _B2) / (np.linalg.norm(_B1) * np.linalg.norm(_B2))
    )

    _B1 = B2 - O2
    _B2 = B3 - O2
    angle2 = (180.0 / np.pi) * np.arccos(
        np.dot(_B1, _B2) / (np.linalg.norm(_B1) * np.linalg.norm(_B2))
    )
...
def add_uci_scale_bar(ax, uci, unit_size=20, unit_name="nm", fontsize=18, *params):
    ...
    scalebar = AnchoredSizeBar(
        ax.transData,
        unit_size / (uci.data.shape[-1] * uci.original_scale),
        str(unit_size) + unit_name,
        "lower right",
        pad=0.1,
        color="white",
        frameon=False,
...
def add_scale_bar(
    ...
    scalebar = AnchoredSizeBar(
        ax.transData,
        unit_size / (s.axes_manager[0].scale),
        str(unit_size) + unit_name,
        "lower right",
        pad=pad,
        color=color,
...
def plot_over_image(
    ...
    t_interp = griddata(
        (x, y), z, (grid_x, grid_y), method=interp_method, fill_value=0.0
    )
