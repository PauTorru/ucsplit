...
class UnitCellImage(hs.signals.Signal2D, RefinePositions):
	def __init__(
		...
		**kwargs,
		...
		if not image is None:
			...
			self.units = image.axes_manager[0].units
		else:
	...
	def build(self, *args, **kwargs):
		...
		self.axes_manager[-1].scale = self.original_scale
		self.axes_manager[-2].scale = self.original_scale
		self.axes_manager[-1].units = self.units
		self.axes_manager[-2].units = self.units
	...
	def uc_add_markers(self):
		...
		for atom in range(self.pos_data.shape[2]):
			...
			marker_pos = self.pos_data[:, :, atom, :].transpose([1, 0, 2])*self.original_scale
