...
class UnitCellImage(hs.signals.Signal2D,RefinePositions):
	...
	def _get_uc_signal(self):
		...
		self.roi_bounds = [i for i in (self.roi.left, self.roi.right, self.roi.top, self.roi.bottom)]
	...
	def map_uc_property(self,function):
		...
		return np.array([function(i) for i in self.pos_data.reshape([x*y,p,c])]).reshape([x,y])
	...
	def calibrate_with_distance_between_equivalents(self,n_atom,direction,slice_range,physical_distance,unit="nm"):
	...
	def reflatten_to_image(self, array = None, default_fill=0.0):
		...
		if is_3d:
			...
			reconstructed[mask, :] = (
				accumulator[mask, :] / counts[mask, np.newaxis]
			)
		else:
...
def load(filename):
	...
	with tempfile.TemporaryDirectory() as tmpdir:
		...
		uci = UnitCellImage(None,uc_centers_matrix=saved_state["uc_centers_matrix"],data=raw_signal.data)
