...
@njit(fastmath=True)
def nb_fit_single_cell_2dgauss(
	cell_image, init_params, n_peaks, max_drift, bounds, max_iter=200, tol=1e-4
):
	...
	for i in range(n_peaks):
		...
		p[idx] = max(px_min, min(px_max, p[idx]))
		p[idx + 1] = max(py_min, min(py_max, p[idx + 1]))
		...
		for i in range(n_peaks):
			...
			for y in range(ny):
				...
				for x in range(nx):
					...
					exp_val = -0.5 * (
						(x_rot * x_rot) * inv_sx2 + (y_rot * y_rot) * inv_sy2
					)
					if exp_val < -700.0:
						exp_val = -700.0
...
def nb_fit_all_cells_2dgauss(
	pos_data, preprocessed_ucs, f, iters, tol, progress_hook, max_drift, global_bounds
):
	...
	for r in prange(n_rows):
		for c in prange(n_cols):
			...
			local_bounds = np.empty((n_atoms, 12), dtype=np.float64)
			local_bounds[:, 2:] = global_bounds[:, 2:]

			local_bounds[:, :2] = global_bounds[:, :2] * scale_factor
			pxs = pos_data[r, c, :, 0]
			pys = pos_data[r, c, :, 1]
			local_bounds[:, 2] = pxs + global_bounds[:, 2]
			local_bounds[:, 3] = pxs + global_bounds[:, 3]
			local_bounds[:, 4] = pys + global_bounds[:, 4]
			local_bounds[:, 5] = pys + global_bounds[:, 5] # px,py are bounds are "drift" bounds (distance from initial position)
			local_bounds[:, 2] = np.maximum(local_bounds[:, 2], 0)
			local_bounds[:, 4] = np.maximum(local_bounds[:, 4], 0)
			local_bounds[:, 3] = np.minimum(local_bounds[:, 3], nx)
			local_bounds[:, 5] = np.minimum(local_bounds[:, 5], ny)
			...
			p_fit = nb_fit_single_cell_2dgauss(
				cell_image,
				p0,
				n_atoms,
				max_drift,
...
class RefinePositions:
	def refine_atom_positions_com(self, iters=5, radius=5):
		...
		refined_positions = nb_refine_all_cells_com(
			pos_matrix, ucs_matrix, float(radius), int(iters)
		)
	...
	def preprocess_tophat(self, ucs_matrix, radius=5, **kwargs):
		...
		for r in range(n_rows):
			for c in range(n_cols):
				processed_data[r, c] = white_tophat(
					ucs_matrix[r, c], footprint=footprint
				)
	...
	def refine_uc_atoms_2dgauss(
		...
		**kwargs,
		...
		b_dict = {
			...
			"sy": (0.1, max_width),
			"theta": (-np.inf, np.inf),
			"px": (-max_drift, +max_drift),
			"py": (-max_drift, +max_drift),
		}

		n_peaks = self.pos_data.shape[2]
		# possible keys: sx,sy,sigma_x,sigma_y,theta,A,sigma,shift,shiftx,shifty,px,py,amplitude,angle,drift
		# shiftx,shiftx are alias for px,py
		# drift overrides px,py
		# shift and sigma override px,py/ sx,sy respectively
		alias_map = {
			"sigma_x": "sx",
			"sigma_y": "sy",
			"amplitude": "A",
		...
		true_params = ["A", "sx", "sy", "px", "py", "theta"]

		conflicting_params_collection = [
			("drift", "px", "shift", "shiftx"),
			("drift", "py", "shift", "shifty"),
			("sigma", "sy", "sigma_y"),
			("sigma", "sx", "sigma_x"),
			("theta", "angle"),
			("amplitude", "A"),
		...
		_bounds = bounds.copy() if bounds is not None else {}

		for conflicting_params in conflicting_params_collection:
			if sum([i in conflicting_params for i in _bounds.keys()]) > 1:
				raise ValueError(f"Conflicting bounds: {conflicting_params}")

		# deal with keywords drift, sigma
		if "drift" in _bounds.keys():
			val = _bounds.pop("drift")
			_bounds["px"] = (-val,val)
			_bounds["py"] = (-val,val)
		if "shift" in _bounds.keys():
			val = _bounds.pop("shift")
			_bounds["px"] = (-val,val)
			_bounds["py"] = (-val,val)
		if "sigma" in _bounds.keys():
			val = _bounds.pop("sigma")
			_bounds["sx"] = val
			_bounds["sy"] = val
		...
		if _bounds is not None:
			for k in _bounds.keys():
				...
				if not isinstance(_bounds[k], Iterable):  # single value, assume is a max:
					if k in alias_map.keys():
						final_key = alias_map[k] + "_max"
					else:
						final_key = k + "_max"

					expanded_bounds[final_key] = np.array(
						n_peaks * [_bounds[k]], dtype=np.float64
					)

				elif len(_bounds[k]) == 2:  # bound is of form (min,max)
					bmin, bmax = _bounds[k]

					if k in alias_map.keys():
						final_key_min = alias_map[k] + "_min"
						final_key_max = alias_map[k] + "_max"
					else:
						final_key_min = k + "_min"
						final_key_max = k + "_max"

					expanded_bounds[final_key_min] = np.array(
						n_peaks * [bmin], dtype=np.float64
					)
					expanded_bounds[final_key_max] = np.array(
						n_peaks * [bmax], dtype=np.float64
					)

				elif len(_bounds[k]) == n_peaks:  # specific bounds for each atom
					k_bounds = np.array(_bounds[k])
					bmin = k_bounds[:, 0]
					bmax = k_bounds[:, 1]
					...
					expanded_bounds[final_key_min] = bmin
					expanded_bounds[final_key_max] = bmax
				else:
					raise ValueError(
						"The bounds were not properly defined. bound[var] must be either: float (sets var maximum),"
						" (min, max) or array of shape (n_atoms,2) where the first columns are the min for each atom and the second one is the max."
					)

		# set the defaults for unset bounds
		for k in true_params:
			default_min, default_max = b_dict[k]

			if k + "_min" not in expanded_bounds.keys():
				expanded_bounds[k + "_min"] = np.array(
					n_peaks * [default_min], dtype=np.float64
				)
			if k + "_max" not in expanded_bounds.keys():
				expanded_bounds[k + "_max"] = np.array(
					n_peaks * [default_max], dtype=np.float64
				)

		for k in expanded_bounds.copy().keys():  # remove unused bounds
			if not any([i in k for i in true_params]):
				del expanded_bounds[k]
		...
		bounds_array = np.vstack([expanded_bounds[k] for k in true_params_minmax]).T
		assert bounds_array.shape == (n_peaks, 12)
		assert (expanded_bounds["px_min"] <= 0).all()
		assert (expanded_bounds["py_min"] <= 0).all()  # ensure relative bounds
